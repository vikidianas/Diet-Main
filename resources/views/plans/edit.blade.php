<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <x-normal-form-section :submit="route('plans.update', $plan)" method="PUT">
                <x-slot:title>
                    Ubah rencana mingguan
                </x-slot>

                <x-slot:description>
                    Memperbarui data rencana mingguan
                </x-slot>

                <x-slot:form>
                    <div class="col-span-6 sm:col-span-4">
                        <x-jet-label for="name" value="{{ __('Nama Rencana') }}" />
                        <x-jet-input id="name" type="text" class="mt-1 block w-full" name="name" :value="old('name', $plan->name)" />
                    ...
                    <div class="col-span-6 sm:col-span-4">
                        ...
                        <x-textarea id="description" class="mt-1 block w-full" name="description" :value="old('description', $plan->description)" />
                        ...
                        <x-select id="recipes[]" class="mt-1 block w-full" name="recipes[]" multiple>
                            <x-slot:options>
                                @foreach ($recipes as $key => $recipe)
                                    <option value="{{ $recipe->id }}" @selected($plan->recipes->contains(old('recipes.'.$key, $recipe->id))) >{{ $recipe->name }}</option>
                                @endforeach
                            </x-slot>
                        </x-select>
